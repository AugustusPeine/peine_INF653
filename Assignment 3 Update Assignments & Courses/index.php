...
switch ($action) {
    ...
    case "delete_assignment":
        ...
        //New case for updating an assignment
    case "update_assignment":
        if ($assignment_id && $description && $course_id) {
            ...
            $error = "Invalid for assignment update.";
            include("view/error.php");
            exit();
        }
        break;
        
        //New case for updating a course
    case "update_course":
        $course_id_post = filter_input(INPUT_POST, 'course_id', FILTER_VALIDATE_INT);
        $course_name_post = filter_input(INPUT_POST, 'course_name', FILTER_SANITIZE_SPECIAL_CHARS);

        if ($course_id_post && $course_name_post) {
            update_course($course_id_post, $course_name_post);
            header("Location: .?action=list_courses");
            exit();
        } else {
            $error = "Invalid for course update.";
            include("view/error.php");
            exit();
        }
        break;
